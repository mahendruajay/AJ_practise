package pattern_based.searching_sorting.ik.part4_ext_heapsort;


/*
Given an array of points where points[i] = [xi, yi] represents a point on the X-Y plane and an integer k, return the k closest points to the origin (0, 0).
The distance between two points on the X-Y plane is the Euclidean distance (i.e., √(x1 - x2)2 + (y1 - y2)2).
You may return the answer in any order. The answer is guaranteed to be unique (except for the order that it is in).

Input: points = [[1,3],[-2,2]], k = 1
Output: [[-2,2]]
Explanation:
The distance between (1, 3) and the origin is sqrt(10).
The distance between (-2, 2) and the origin is sqrt(8).
Since sqrt(8) < sqrt(10), (-2, 2) is closer to the origin.
We only want the closest k = 1 points from the origin, so the answer is just [[-2,2]].

Example 2:

Input: points = [[3,3],[5,-1],[-2,4]], k = 2
Output: [[3,3],[-2,4]]
Explanation: The answer [[-2,4],[3,3]] would also be accepted.


 */


import java.util.Arrays;
import java.util.Comparator;
import java.util.PriorityQueue;
import java.util.Queue;

public class KClosetPointsToOrigin {

    Queue<Point> maxHeap = new PriorityQueue<>(new PointComparator());

    public static void main(String[] args) {
        int[][] input = new int[][]{{3, 3}, {5, 1}, {-2, 4}};
        KClosetPointsToOrigin kClosetPointsToOrigin = new KClosetPointsToOrigin();
        for (int[] arr : kClosetPointsToOrigin.kClosest(input, 2)) {
            System.out.println(Arrays.toString(arr));
        }

    }

    public int[][] kClosest(int[][] points, int k) {

        int[][] result = new int[k][];
        for (int i = 0; i < points.length; i++) {
            Point curr = new Point(points[i][0], points[i][1]);
            if (maxHeap.size() < k) {
                maxHeap.offer(curr);
            } else if (maxHeap.size() == k && curr.distFromOrigin < maxHeap.peek().distFromOrigin) {
                maxHeap.poll();
                maxHeap.offer(curr);
            }
        }

        int i = 0;
        for (Point p : maxHeap) {
            result[i] = new int[]{p.x, p.y};
            i++;
        }

        return result;

    }

    public class Point {
        int x;
        int y;
        double distFromOrigin;

        public Point(int x, int y) {
            this.x = x;
            this.y = y;
            this.distFromOrigin = Math.sqrt(((x - 0) * (x - 0)) + (y - 0) * (y - 0));
        }

    }

    public class PointComparator implements Comparator<Point> {
        public int compare(Point point1, Point point2) {
            return Double.compare(point2.distFromOrigin, point1.distFromOrigin);
        }
    }
}
